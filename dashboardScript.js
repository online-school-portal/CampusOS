...
async function updateStudent(id, payload) {
 ...
 /*async function deleteStudent(studentIdEncoded) {
  ...
  try {
    // Delete the student row from the "students" table
    const { error } = await supabaseClient
      .from("students")
      .delete()
      .eq("student_id", studentId); // use the correct primary/key column

    if (error) throw error;

    alert("Student deleted successfully!");
    // Optionally, refresh your student list
    await fetchStudents();
...
async function loadSubscriptionUI() {
  try {
    const schoolRes = await supabaseClient.rpc("current_user_school_id");
    const schoolId = schoolRes.data;
