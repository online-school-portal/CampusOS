...
form.dataset.authId = teacher.auth_id ?? "";
...
document.getElementById("editTeacherForm").addEventListener("submit", async (e) => {
  ...
  if (!uuid) {
    return alert("Missing teacher ID for update");
  }
  ...
  const payload = {
    ...
    class_teacher:
      form.classTeacher.value.trim() || null,
  ...
  try {
    ...
    if (
      ...
      if (!authRes.ok) {
        throw new Error(
          authResult.error ||
          "Failed to update teacher login"
        );
      }

      // If login was newly created,
      // save new auth_id locally.
